...
on_hours = ['08','09','10','11','12','13','14','15','16','17','18']
slides = []
slide_time = 10
...
def during_the_day():
	# posts move to daytime in event_log.txt file
	f = open('event_log.txt', 'a')
	time_now = datetime.datetime.now().strftime('%c')
	f.write(time_now + ' - Moved to day time\n')
	f.close()
...
def during_the_night():
	# posts move to nightime in event_log.txt file
	f = open('event_log.txt', 'a')
	time_now = datetime.datetime.now().strftime('%c')
	f.write(time_now + ' - Moved to night time\n')
	f.close()
	# defines a variable to track changes to black slide
	slide_is_black = False
	# loop that runs forever
	while True:
		...
		if the_hour == '00' and the_min == '00':
			# posts about to reboot in event_log.txt file
			f = open('event_log.txt', 'a')
			time_now = datetime.datetime.now().strftime('%c')
			f.write(time_now + ' - About to reboot\n')
			f.close()			
...
# posts successful reboot in event_log.txt file
f = open('event_log.txt', 'a')
time_now = datetime.datetime.now().strftime('%c')
f.write(time_now + ' - Successful reboot\n')
f.close()
# starts assuming it is night time
during_the_night()
